...
@interface M3TravelViewController ()<UIWebViewDelegate,UIAlertViewDelegate>
...
@implementation M3TravelViewController
...
- (void)viewDidLoad
{
    ...
    _urlStirng=[NSString stringWithFormat:@"%@LoginName=%@&Token=%@",TravelUrl,[UserInfo sharedUserInfo].employeeNo,[UserInfo sharedUserInfo].token];
...
//                    [self performSelector:@selector(dismissHUD) withObject:@"dismissHUD" afterDelay:30];
            ...
            if ([funcStr isEqualToString:@"showAlert"]) {
                ...
                if ([arrFucnameAndParameter objectAtIndex:1]) {
                    ...
                    UIAlertView* alert = [[UIAlertView alloc] initWithTitle:[[arrFucnameAndParameter objectAtIndex:1] URLDecodedString] message:nil delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Confirm", nil];
                    [alert show];
